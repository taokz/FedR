...
class SpGAT(nn.Module):
    def __init__(self, num_nodes, nfeat, nhid, relation_dim, dropout, alpha, nheads, gpu):
        """
    ...
    def forward(self, Corpus_, batch_inputs, entity_embeddings, relation_embed,
        ...
        relation_embed = relation_embed.cuda()
        edge_embed_nhop = relation_embed[
            edge_type_nhop[:, 0]] + relation_embed[edge_type_nhop[:, 1]].cuda()
...
class SpKBGATModified(nn.Module):
    def __init__(self, initial_entity_emb, initial_relation_emb, entity_out_dim, relation_out_dim,
        ...
        self.sparse_gat_1 = SpGAT(self.num_nodes, self.entity_in_dim, self.entity_out_dim_1, self.relation_dim,
                                  self.drop_GAT, self.alpha, self.nheads_GAT_1, self.gpu)
    ...
    def forward(self, Corpus_, adj, batch_inputs, train_indices_nhop):
        ...
        edge_list = adj[0]
        edge_type = adj[1]
        # assert not torch.isnan(edge_list).any()
        ...
        edge_embed = self.relation_embeddings[edge_type].cuda()
        ...
        # mask_indices = torch.unique(batch_inputs[:, 2]).to(self.gpu)
        # mask = torch.zeros(self.entity_embeddings.shape[0]).to(self.gpu)
...
class SpKBGATConvOnly(nn.Module):
    ...
    def batch_test(self, batch_inputs):
        #print(self.final_relation_embeddings.device, self.final_relation_embeddings.data.device, self.final_entity_embeddings.data.device, type(self.final_relation_embeddings.data), batch_inputs.device)
        #self.final_relation_embeddings = self.final_relation_embeddings.cuda()
        conv_input = torch.cat((self.final_entity_embeddings[batch_inputs[:, 0], :].unsqueeze(1), self.final_relation_embeddings[
            batch_inputs[:, 1]].unsqueeze(1), self.final_entity_embeddings[batch_inputs[:, 2], :].unsqueeze(1)), dim=1)
        out_conv = self.convKB(conv_input)
        return out_conv
