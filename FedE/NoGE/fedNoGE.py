...
class Server(object):
    def __init__(self, args, nentity):
        self.args = args
        self.entity_embeddings = torch.Tensor(np.zeros((nentity, args.emb_dim))).to(args.gpu)
        self.nentity = nentity

    def send_emb(self, init):
        if self.args.isSecure == 1:
            if init == 0:
                server_ent_emb = copy.deepcopy(self.entity_embeddings)
            elif init == 1:
                server_ent_emb = copy.deepcopy(self.entity_embeddings)
                server_ent_emb = server_ent_emb / (10**self.args.quantization_bit)
        else:
            server_ent_emb = copy.deepcopy(self.entity_embeddings)
        return server_ent_emb
    ...
    def aggregation(self, clients, ent_update_weights):
        ...
        update_ent_embed = torch.Tensor(np.zeros((self.nentity, self.args.emb_dim))).to(self.args.gpu)

        # PSI
        if self.args.isSecure == 2:
            mask_index = [np.nonzero(ent_w[i]).cpu().detach().numpy() for i in range(self.args.num_client)]
            mask_psi_ind = reduce(np.intersect1d, mask_index)

        for i, client in enumerate(clients):
            local_ent_embed = client.ent_embed.clone().detach()
            if self.args.isSecure == 1:
                local_ent_embed = (local_ent_embed * (10**self.args.quantization_bit)).long().float()
            update_ent_embed += local_ent_embed * ent_w[i].reshape(-1, 1)
        
        if self.args.isSecure == 2:
            self.entity_embeddings[mask_psi_ind] = update_ent_embed[mask_psi_ind]
        else:
...
class Client(object):
    ...
    def client_update(self):
        ...
        self.ent_embed = copy.deepcopy(self.kge_model.embeddings.weight.data[:self.nentity])
        self.rel_embed = copy.deepcopy(self.kge_model.embeddings.weight.data[self.nentity:])
    ...
    def client_eval(self, istest=False):
        ...
        self.kge_model.eval()
        self.kge_model.embeddings.weight.data[:self.nentity] = self.ent_embed
        self.kge_model.embeddings.weight.data[self.nentity:] = self.rel_embed
        with torch.no_grad():
            hits = []
            ranks = []
            for i in range(10):
...
class fedNoGE(object):
    ...
    def send_emb(self, init):
        for k, client in enumerate(self.clients):
            client.ent_embed = self.server.send_emb(init)
